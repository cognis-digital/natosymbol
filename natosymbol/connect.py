"""Native cognis-connect emit for natosymbol — forward findings to any platform.

Maps natosymbol's JSON output to the canonical `Finding` and forwards it via
...
Usage:
    natosymbol ... --format json | natosymbol-emit --to stix
    natosymbol-emit --to slack --url $WEBHOOK --dry-run < findings.json
"""
...
def map_record(rec: dict) -> dict:
    ...
    try:
        out = dict(rec)
        out['title'] = rec.get('symbol', '') + ' ' + rec.get('category', '')
